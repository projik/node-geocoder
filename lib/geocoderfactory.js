	...
	var GeocoderFactory = {
		...
		*/
		_getGeocoder: function(geocoderName, adapter, extra) {
			...
			if (geocoderName === 'bing') {
				var BingGeocoder = new require('./geocoder/binggeocoder.js');

				return new BingGeocoder(adapter, extra.apiKey);
			}
			if (geocoderName === 'openmapquest') {
				var MapQuestGeocoder = new require('./geocoder/openmapquestgeocoder.js');
